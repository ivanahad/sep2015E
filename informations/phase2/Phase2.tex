...
During the previous phase, we decided not to implement one of the requirements (\textit{After a pool has been generated, a staff member can manually reorganize it}). As we had a very busy phase, we came to the conclusion that it was not necessary to implement it because the benefits were not worth it compared to the time it would take us to do it. In this phase however, we made sure to implement it and all the requirements we had intended to develop. Here is the list of them : \\
